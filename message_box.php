...
<?php
    ...
    if ($mode=="send") {
        $sql = "select * from message where send_id='$uid' order by num desc";
    } else {
        $sql = "select * from message where rv_id='$uid' order by num desc";
    }
   ...
   for ($i=$start; $i<$start+$scale && $i < $total_record; $i++) {
       ...
       $result2 = mysqli_query($con, "select uname from members where uid='$msg_id'");
       $record = mysqli_fetch_array($result2);
       $msg_name = $record["uname"]; ?>
				<li>
